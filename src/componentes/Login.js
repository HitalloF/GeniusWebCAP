...
function Login() {
  ...
  return (
    <div className="tw-flex tw-flex-col tw-items-center tw-justify-center tw-min-h-screen" style={{ backgroundImage: `url(${bh})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
      ...
      <h1 className="tw-text-white tw-text-6xl tw-drop-shadow-sm tw-font-bold tw-mb-10">{HomeTitle}</h1>
      <div className="tw-bg-black tw-bg-opacity-75 tw-p-8 tw-rounded-lg tw-shadow-lg">

        <h2 className="tw-text-2xl tw-font-bold tw-text-white">{login}</h2>
        <form onSubmit={handleSubmit} className="tw-w-full tw-max-w-xs">
          ...
          <div className="tw-flex tw-justify-center">
            <button type="submit" className="tw-bg-blue-500 tw-hover:tw-bg-blue-700 tw-text-white tw-font-bold tw-py-2 tw-px-4 tw-rounded tw-focus:outline-none tw-focus:shadow-outline">{submit}</button>
          </div>
        </form>
